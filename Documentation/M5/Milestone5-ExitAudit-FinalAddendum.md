...
### 1. Backend test project could not compile _(original Blocker 1)_
...
**Consequence worth recording:** the 16 functional tests had _never executed
once_. They arrived in the same upload as the duplicate that blocked
...
### 2. Order status casing broke admin order management _(original Blocker 2)_
...
Six assertions across two files encoded the _old_ casing and were corrected —
...
### 3. The concurrency token did nothing on PostgreSQL _(new — production defect)_
...
### 4. Auth rate limiting was global, not per-IP _(new — production defect)_
...
Milestone 1 section 12 specified `express-rate-limit`, which is **per-IP by default**;
...
### 5. Test host could not run transactions _(test infrastructure, not a defect)_
...
`status` is the _number_ 500) into `OrderResponse`.
...
- [ ] Add a GitHub Actions workflow — **still absent**, and M1 section 15.2 requires it
...
no category update endpoint; orders paging omits `page`/`limit`; order-item product _name_
