...
| E2E (Playwright/Cypress) | ❌ **No** | — | — | M1 §2.3 lists E2E in the stack; no E2E tooling exists in the repo |
...
| Product management | ⚠️ **Incomplete** | Create + soft-delete work; **update has no UI** though `PUT /api/products/{id}` exists (T-17 is a *Must*) |
...
Middleware order in `Program.cs` matches the v1.1 §2.4 diagram: exception handling →
...
Resource-ownership authorization lives in the service layer exactly as v1.1 §185 requires
...
**Schema verified statically against M1 §8.4. Migration execution was not performed** —
...
| Planned indexes (M1 §8.3) | ⚠️ `users.email`, `products.category_id`, `orders.user_id` present; **`products.is_active` index missing** |
...
| Historical product **name** | ⚠️ **Not** snapshotted — read live from `Product.Name`. M1 §8.2 specifies a snapshot of "product name/price". A rename rewrites history |
| Category delete behaviour | ⚠️ DB uses `SetNull`; M1 §8.2 specifies `RESTRICT`. **Behaviour is still correct** — `CategoryService` blocks deletion with `409` when active products reference the category |
...
| M1 §9.2 / §15 describe a Node.js + Express backend | **Historical and acceptable** — formally superseded by `M1_SystemPlan_v1.1.md`, which is correctly written as an amendment |
| M1 §8 references Prisma | **Historical and acceptable** — same reason |
| M1 §8.2 specifies order items snapshot product *name*; implementation snapshots price only | Should fix (doc or code) |
| M1 §8.2 specifies category FK `RESTRICT`; implementation uses `SetNull` + service-level `409` | Documentation-only (behaviour meets T-19) |
...
| JWT authentication | M1 §12 | `TokenService`, JWT Bearer middleware | **Yes** — code path traced; claim shape matches spec | ✅ Complete |
| MVC + Service + Repository | M1 §6.1 / v1.1 §2.2 | Backend | **Yes** — mechanically checked, 0 violations | ✅ Complete |
| Server-side role authorization | M1 §12 | `[Authorize(Roles)]` + ownership checks | **Yes** | ✅ Complete |
| Soft delete (T-18) | M1 §8 | Global query filter | **Yes** | ✅ Complete |
| Atomic checkout (NFR Reliability) | M1 §4 | `OrderRepository.CreateFromCartAsync` | **Yes** — single transaction, rollback on failure | ✅ Complete |
| Order state machine (T-21) | M1 §3.5 | `OrderService.AllowedTransitions` | **Yes** — backend correct; **UI broken** | ⚠️ Blocker 2 |
| Update product (T-17, *Must*) | M1 §3.5 | API exists, **no UI** | **Yes** | ⚠️ Incomplete |
| Profile with address (T-14) | M1 §3.5 | `addresses` table exists; no endpoints or UI | **Yes** | ⚠️ Partial (name only) |
| Price filter (T-06, *Should*) | M1 §3.5 | Absent both sides | **Yes** | ⚠️ Not implemented |
...
| E2E testing | M1 v1.1 §2.3 | None | **Yes** | ❌ Absent |
| CI/CD (GitHub Actions) | M1 §15.2 / v1.1 | **No CI configuration exists in the repository** | **Yes** | ❌ Absent |
...
(`dotnet test`, v1.1 §1.8) cannot be established in M6.
...
machine. This breaks T-20 and T-21, both *Must* requirements, and makes the admin order
...
frontend's *assumed* capitalised casing rather than the backend's actual output — a
...
1. **No CI/CD configuration exists.** M1 §15.2 and v1.1 §1.8 both specify GitHub Actions running `dotnet test`. There is no `.github/` directory. Required for M6's pipeline.
2. **No E2E tooling.** M1 v1.1 §2.3 lists Playwright or Cypress. The user-journey test is React Testing Library, which is not E2E.
...
5. **T-17 (Update product, *Must*) has no UI.** `productApi.update` exists and is wired to `PUT /api/products/{id}`, but no component calls it. An admin cannot edit a product.
...
7. **T-06 (filter by price, *Should*) is not implemented** on either side.
...
11. **Order item product *name* is not snapshotted** (only price is). M1 §8.2 specifies both. Renaming a product rewrites the name shown on historical orders.
12. **`products.is_active` index from M1 §8.3 was never created.**
13. **Category FK uses `SetNull`,** where M1 §8.2 specifies `RESTRICT`. Behaviour is nonetheless correct because the service blocks deletion with a `409`.
